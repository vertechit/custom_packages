...
class InputImage {
  ...
  InputImage._({this.filePath, this.bytes, @required this.type, this.inputImageData});
  ...
  factory InputImage.fromBytes({@required Uint8List bytes, @required InputImageData inputImageData}) {
    return InputImage._(bytes: bytes, type: InputImageType.bytes, inputImageData: inputImageData);
  }

  /// Returns a json representation of an instance of [InputImage].
  Map<String, dynamic> toJson() => {'bytes': bytes, 'type': type.name, 'path': filePath, 'metadata': inputImageData == null ? 'none' : inputImageData.toJson()};
}
...
class InputImageData {
  ...
  Map<String, dynamic> toJson() => {
        ...
        'planeData': planeData?.map((InputImagePlaneMetadata plane) => plane.toJson()).toList(),
      };
}
...
extension InputImageRotationValue on InputImageRotation {
  ...
  static InputImageRotation fromRawValue(int rawValue) {
    try {
      return InputImageRotation.values.firstWhere((element) => element.rawValue == rawValue);
    } catch (_) {
      return null;
    }
  }
