...
abstract class IDetectFace {
  // InputImage inputImage;
  Future<List<Face>> call({@required CameraImage image, @required InputImage inputImage});
  Future close();
}

class DetectFace implements IDetectFace {
  ...
  bool _isBusy = false;
  @override
  Future<List<Face>> call({@required CameraImage image, @required InputImage inputImage}) async {
    ...
    _faceDetector ??= _faceDetector = FaceDetector(
      options: FaceDetectorOptions(
        enableContours: true,
        enableClassification: true,
      ),
...
//         landmarks: Map<FaceLandmarkType, FaceLandmark>.fromIterables(FaceLandmarkType.values, FaceLandmarkType.values.map((FaceLandmarkType type) {
...
//                 );
//         })),
//         contours: Map<FaceContourType, FaceContour>.fromIterables(FaceContourType.values, FaceContourType.values.map((FaceContourType type) {
//           /// added empty map to pass the tests
//           final List<dynamic> arr = (json['contours'] ?? <String, dynamic>{})[type.name];
//           return (arr == null)
//               ? null
//               : FaceContour(
//                   type: type,
//                   points: arr.map<Point<int>>((dynamic pos) => Point<int>(pos[0].toInt(), pos[1].toInt())).toList(),
//                 );
//         })),
//       );
// }
